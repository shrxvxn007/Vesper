"""End-to-end integration test: drives ``vesper.main.run_backtest`` and asserts the
...
# ``vesper`` is importable via ``pythonpath = .`` in ``pytest.ini``; no per-test
# ``sys.path`` hack required.


def test_run_backtest_end_to_end(tmp_path: Path) -> None:
    from vesper.main import run_backtest
...
def test_run_backtest_end_to_end_xgboost(tmp_path: Path) -> None:
    """Same institutional invariants as ``test_run_backtest_end_to_end`` but with
    ``model_type='xgboost'`` so the XGBoost backend is exercised end-to-end."""
    pytest.importorskip("xgboost")
    from vesper.main import run_backtest

    data_dir = tmp_path / "data_xgb"
    result = run_backtest(data_dir=str(data_dir), model_type="xgboost")
    ...
    # Diagnostics sanity: nonce-cancel pnl must be finite; cap binding must respect ±3%.
    assert np.isfinite(result.diagnostics["avg_weekly_net_pnl"])
    assert result.diagnostics["max_gross_weight"] <= 0.03 + 1e-4
...
def test_main_py_module_is_self_contained(tmp_path: Path) -> None:
    ...
    import vesper.main  # noqa: F401

    data_dir = tmp_path / "data"
    # The CLI main runs sys.exit(0) on success.
    from vesper.main import main as cli
